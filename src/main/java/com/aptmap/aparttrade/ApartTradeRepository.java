...
public interface ApartTradeRepository extends JpaRepository<AptTrade, Long> {
    ...
    // 시군구 코드 + 거래 날짜 범위로 조회
   // 좌표 없는 데이터는 제외 → 카카오 주소 변환 실패 항목은 지도에 표시 안 함
    @Query("SELECT a FROM AptTrade a WHERE a.sggCd = :sggCd AND a.dealDate BETWEEN :from AND :to AND a.latitude IS NOT NULL AND a.longitude IS NOT NULL")
